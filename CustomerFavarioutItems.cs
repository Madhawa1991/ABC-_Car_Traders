        ...
        private void CustomerFavorioteConfirmOrderBtn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    // Update the OrderStatus to "Ordered" and add the customer comment and order date
                    string query = @"
                UPDATE [order]
                ...
                WHERE OrderID = @OrderID";
                    ...
                    {
                        command.Parameters.AddWithValue("@OrderID", OrderID);
                        command.Parameters.AddWithValue("@CustomerComment", customerComment ?? (object)DBNull.Value); // Use DBNull whn Value if comment is null
                        ...
                        if (rowsAffected > 0)
                        {
                            ...
                            LoadData(); // Call LoadData to refresh the grid view
                            CustomerFavorioteClearBtn_Click(sender,e);//calling clear btn
                        }
                        else
                        {
